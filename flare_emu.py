...
class EmuHelper:
  ...
  def __init__(self, verbose=0, emuHelper=None, samplePath=None, isRizin=False):
    ...
    if isRizin:
      ...
      if emuHelper is not None:
        ...
        self.analysisHelper = flare_emu_radare.Radare2AnalysisHelper(
            samplePath, self
        )
      ...
      try:
        ...
        self.logger.error(
            "error importing flare_emu_ida: specify samplePath to use radare2"
            " or run under IDA Pro 7+"
        )
        return
  ...
  def emulateRange(
    ...
    userData = {
        "EmuHelper": self,
        "funcStart": self.analysisHelper.getFuncStart(startAddr),
        "funcEnd": self.analysisHelper.getFuncEnd(startAddr),
        "skipCalls": skipCalls,
        "strict": strict,
        "endAddr": endAddr,
        "callHook": callHook,
    ...
    if instructionHook:
      self.h_userhook = mu.hook_add(
          unicorn.UC_HOOK_CODE, instructionHook, userData
      )
    if memAccessHook:
      self.h_memaccesshook = self.uc.hook_add(
          unicorn.UC_HOOK_MEM_READ | unicorn.UC_HOOK_MEM_WRITE,
          memAccessHook,
          userData,
      )
    self.h_memhook = mu.hook_add(
        unicorn.UC_HOOK_MEM_READ_UNMAPPED
        | unicorn.UC_HOOK_MEM_WRITE_UNMAPPED
        | unicorn.UC_HOOK_MEM_FETCH_UNMAPPED,
        self._hookMemInvalid,
        userData,
    )
  ...
  def iterate(
    ...
    if type(target) in [int, long]:
      self.logger.debug("iterate target function: %s" % self.hexString(target))
      ...
      for i, x in enumerate(xrefs):
        ...
        if funcStart == None:
          continue
        if self.analysisHelper.getMnem(x).upper() not in [
            "CALL",
            "JMP",
            "BL",
            "BLX",
    ...
    if instructionHook:
      self.h_userhook = self.uc.hook_add(
          unicorn.UC_HOOK_CODE, instructionHook, userData
      )
    if memAccessHook:
      self.h_memaccesshook = self.uc.hook_add(
    ...
    self.h_memhook = self.uc.hook_add(
        unicorn.UC_HOOK_MEM_READ_UNMAPPED
        | unicorn.UC_HOOK_MEM_WRITE_UNMAPPED
        | unicorn.UC_HOOK_MEM_FETCH_UNMAPPED,
        self._hookMemInvalid,
    ...
    while len(userData["targetInfo"]) > 0:
      ...
      userData["targetVA"] = targetVA = sorted(
          userData["targetInfo"].keys(), reverse=True
      )[0]
      ...
      self.logger.debug(
          "iterate run #%d, %d targets remaining: %s (%d paths)"
      ...
      for path in paths:
        self.logger.debug(
            "emulating path #%d of %d from %s to %s via basic blocks: %s"
            % (
                cnt2,
                numPaths,
                self.hexString(funcStart),
                self.hexString(targetVA),
                repr(path),
            )
        )
  ...
  def iterateAllPaths(
      ...
      hookApis=True,
      memAccessHook=None,
      maxPaths=MAXCODEPATHS,
      maxNodes=MAXNODESEARCH,
  ):
    flowchart = self.analysisHelper.getFlowChart(target)
    # targets are all function ends
    targets = [
        self.analysisHelper.getBlockEndInsnAddr(bb.start_ea, flowchart)
        for bb in self.analysisHelper.getTerminatingBBs(flowchart)
    ]

    targetInfo = {}
    for i, t in enumerate(targets):
      self.logger.debug(
          "getting paths to %s, %d of %d targets"
    ...
    if len(targetInfo) <= 0:
      self.logger.debug(
          "iterateAllPaths: no targets to iterate for %s"
    ...
    for targetVA in sorted(userData["targetInfo"].keys(), reverse=True):
      ...
      numTargets = len(userData["targetInfo"])
      self.logger.debug(
          "run #%d, %d targets remaining: %s (%d paths)"
          % (cnt, numTargets, self.hexString(targetVA), len(paths))
      )
      cnt2 = 1
      numPaths = len(paths)
  ...
  def writeEmuMem(self, addr, data):
    ...
    if not isinstance(data, bytes):
      self.logger.error(
          "wrong type for writeEmuMem data, expects bytes or bytearray: %s"
          % type(data)
      )
      return
  ...
  def isRetInstruction(self, addr):
    ...
    if (
        self.analysisHelper.getMnem(addr).lower() in ["bx", "b"]
        and self.analysisHelper.getOperand(addr, 0).lower() == "lr"
    ):
      return True

    if (
        self.analysisHelper.getMnem(addr).lower() == "pop"
        and "pc" in self.analysisHelper.getDisasmLine(addr).lower()
    ):
      return True
  ...
  def skipInstruction(self, userData, useAnalysisHelper=False):
    ...
    self.uc.reg_write(
        self.regs["sp"],
        self.getRegVal("sp")
        + self.analysisHelper.getSpDelta(
            userData["currAddr"]
            + self.analysisHelper.getInsnSize(userData["currAddr"])
        ),
    )
  ...
  def stopEmulation(self, userData):
    self.enteredBlock = False
    if (
        "visitedTargets" in userData
        and userData["targetVA"] not in userData["visitedTargets"]
    ):
      userData["visitedTargets"].append(userData["targetVA"])
    self.uc.emu_stop()

  def resetEmuHooks(self):
    if self.uc is None:
      self.logger.debug(
          "resetEmuHooks: no hooks to reset, emulator has not been"
          " initialized yet"
      )
      return
    if self.h_userhook:
    ...
    if self.arch == unicorn.UC_ARCH_X86:
      if self.uc._mode == unicorn.UC_MODE_64:
        out = "RAX: %016X\tRBX: %016X\n" % (
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_RAX),
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_RBX),
        )
        out += "RCX: %016X\tRDX: %016X\n" % (
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_RCX),
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_RDX),
        )
        out += "RDI: %016X\tRSI: %016X\n" % (
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_RDI),
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_RSI),
        )
        out += "R8: %016X\tR9: %016X\n" % (
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_R8),
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_R9),
        )
        ...
        out += "ECX: %016X\tEDX: %016X\n" % (
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_ECX),
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_EDX),
        )
        out += "EDI: %016X\tESI: %016X\n" % (
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_EDI),
            self.uc.reg_read(unicorn.x86_const.UC_X86_REG_ESI),
        )
      ...
      out = "X0: %016X\tX1: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X0),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X1),
      )
      out += "X2: %016X\tX3: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X2),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X3),
      )
      out += "X4: %016X\tX5: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X4),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X5),
      )
      out += "X6: %016X\tX7: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X6),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X7),
      )
      out += "X8: %016X\tX9: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X8),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X9),
      )
      out += "X10: %016X\tX11: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X10),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X11),
      )
      out += "X12: %016X\tX13: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X12),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X13),
      )
      out += "X14: %016X\tX15: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X14),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X15),
      )
      out += "X16: %016X\tX17: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X16),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X17),
      )
      out += "X18: %016X\tX19: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X18),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X19),
      )
      out += "X20: %016X\tX21: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X20),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X21),
      )
      out += "X22: %016X\tX23: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X22),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X23),
      )
      out += "X24: %016X\tX25: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X24),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X25),
      )
      out += "X26: %016X\tX27: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X26),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X27),
      )
      out += "X28: %016X\tX29: %016X\n" % (
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X28),
          self.uc.reg_read(unicorn.arm64_const.UC_ARM64_REG_X29),
      )
      out += "X30: %016X\n" % (
      ...
      out = "R0: %08X\tR1: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R0),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R1),
      )
      out += "R2: %08X\tR3: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R2),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R3),
      )
      out += "R4: %08X\tR5: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R4),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R5),
      )
      out += "R6: %08X\tR7: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R6),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R7),
      )
      out += "R8: %08X\tR9: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R8),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R9),
      )
      out += "R10: %08X\tR11: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R10),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R11),
      )
      out += "R12: %08X\tR13: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R12),
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R13),
      )
      out += "R14: %08X\tR15: %08X\n" % (
          self.uc.reg_read(unicorn.arm_const.UC_ARM_REG_R14),
  ...
  def getEmuPtr(self, va):
    return struct.unpack(
        self.pack_fmt, self.uc.mem_read(va, self.size_pointer)
    )[0]
  ...
  def getSignedValue(self, value):
    return struct.unpack(
        self.pack_fmt_signed, struct.pack(self.pack_fmt, value)
    )[0]
  ...
  def getPath(self, targetVA):
    ...
    if target_bb.id != 0:
      ...
      if graph is None:
        self.logger.debug(
            "graph for target %s could not be traversed, skipping"
            % self.hexString(targetVA)
        )
        return None, None
      ...
      if not self._findPathFromGraph(path, graph, 0, target_bb.id):
        self.logger.debug(
            "path for target %s could not be discovered, skipping"
            % self.hexString(targetVA)
        )
        return None, None
    else:
  ...
  def getPathsToTarget(
      self, targetVA, maxPaths=MAXCODEPATHS, maxNodes=MAXNODESEARCH
  ):
    flowchart = self.analysisHelper.getFlowChart(targetVA)
    target_bb = self.analysisHelper.getBlockByVA(targetVA, flowchart)
    start_bb = self.analysisHelper.getStartBB(targetVA, flowchart)
    if target_bb.id != 0:
      ...
      self._findPathsFromGraph(
          paths, path, graph, 0, targets, maxPaths, 0, maxNodes
      )
      if len(paths) == 0:
        self.logger.debug(
            "path for target %s could not be discovered, skipping"
  ...
  def getPaths(self, fva, maxPaths=MAXCODEPATHS, maxNodes=MAXNODESEARCH):
    ...
    term_bbs_ids = [
        bb.id for bb in self.analysisHelper.getTerminatingBBs(flowchart)
    ]
    ...
    if term_bbs_ids != [0]:
      ...
      paths = []
      self._findPathsFromGraph(
          paths, path, graph, 0, term_bbs_ids, maxPaths, 0, maxNodes
      )
      if len(paths) == 0:
        self.logger.debug(
            "paths for target %s could not be discovered, skipping"
            % self.hexString(fva)
        )
        return None, None
    else:
  ...
  def initEmuHelper(self):
    ...
    if arch == "X86":
      ...
      if bitness == 64:
        ...
        self.regs = {
            "ax": unicorn.x86_const.UC_X86_REG_RAX,
            "bx": unicorn.x86_const.UC_X86_REG_RBX,
            "cx": unicorn.x86_const.UC_X86_REG_RCX,
            "dx": unicorn.x86_const.UC_X86_REG_RDX,
            "di": unicorn.x86_const.UC_X86_REG_RDI,
            "si": unicorn.x86_const.UC_X86_REG_RSI,
            "bp": unicorn.x86_const.UC_X86_REG_RBP,
            "sp": unicorn.x86_const.UC_X86_REG_RSP,
            "eax": unicorn.x86_const.UC_X86_REG_RAX,
            "ebx": unicorn.x86_const.UC_X86_REG_RBX,
            "ecx": unicorn.x86_const.UC_X86_REG_RCX,
            "edx": unicorn.x86_const.UC_X86_REG_RDX,
            "edi": unicorn.x86_const.UC_X86_REG_RDI,
            "esi": unicorn.x86_const.UC_X86_REG_RSI,
            "ebp": unicorn.x86_const.UC_X86_REG_RBP,
            "esp": unicorn.x86_const.UC_X86_REG_RSP,
            "ip": unicorn.x86_const.UC_X86_REG_RIP,
            "pc": unicorn.x86_const.UC_X86_REG_RIP,
            ...
            "r8d": unicorn.x86_const.UC_X86_REG_R8,
            "r9d": unicorn.x86_const.UC_X86_REG_R9,
            "r10d": unicorn.x86_const.UC_X86_REG_R10,
            "r11d": unicorn.x86_const.UC_X86_REG_R11,
            "r12d": unicorn.x86_const.UC_X86_REG_R12,
            "r13d": unicorn.x86_const.UC_X86_REG_R13,
            "r14d": unicorn.x86_const.UC_X86_REG_R14,
            "r15d": unicorn.x86_const.UC_X86_REG_R15,
            "r8w": unicorn.x86_const.UC_X86_REG_R8,
            "r9w": unicorn.x86_const.UC_X86_REG_R9,
            "r10w": unicorn.x86_const.UC_X86_REG_R10,
            "r11w": unicorn.x86_const.UC_X86_REG_R11,
            "r12w": unicorn.x86_const.UC_X86_REG_R12,
            "r13w": unicorn.x86_const.UC_X86_REG_R13,
            "r14w": unicorn.x86_const.UC_X86_REG_R14,
            "r15w": unicorn.x86_const.UC_X86_REG_R15,
            "r8b": unicorn.x86_const.UC_X86_REG_R8,
            "r9b": unicorn.x86_const.UC_X86_REG_R9,
            "r10b": unicorn.x86_const.UC_X86_REG_R10,
            "r11b": unicorn.x86_const.UC_X86_REG_R11,
            "r12b": unicorn.x86_const.UC_X86_REG_R12,
            "r13b": unicorn.x86_const.UC_X86_REG_R13,
            "r14b": unicorn.x86_const.UC_X86_REG_R14,
            "r15b": unicorn.x86_const.UC_X86_REG_R15,
        ...
        if self.filetype == "PE":
          ...
          self.regs.update({
              "arg1": unicorn.x86_const.UC_X86_REG_RCX,
          ...
          self.regs.update({
              "arg1": unicorn.x86_const.UC_X86_REG_RDI,
        ...
        self.regs = {
            "ax": unicorn.x86_const.UC_X86_REG_EAX,
            "bx": unicorn.x86_const.UC_X86_REG_EBX,
            "cx": unicorn.x86_const.UC_X86_REG_ECX,
            "dx": unicorn.x86_const.UC_X86_REG_EDX,
            "di": unicorn.x86_const.UC_X86_REG_EDI,
            "si": unicorn.x86_const.UC_X86_REG_ESI,
            "bp": unicorn.x86_const.UC_X86_REG_EBP,
            "sp": unicorn.x86_const.UC_X86_REG_ESP,
            "ip": unicorn.x86_const.UC_X86_REG_EIP,
            "pc": unicorn.x86_const.UC_X86_REG_EIP,
      ...
      if bitness == 64:
        ...
        self.regs = {
            ...
            "W0": unicorn.arm64_const.UC_ARM64_REG_X0,
            "W1": unicorn.arm64_const.UC_ARM64_REG_X1,
            "W2": unicorn.arm64_const.UC_ARM64_REG_X2,
            "W3": unicorn.arm64_const.UC_ARM64_REG_X3,
            "W4": unicorn.arm64_const.UC_ARM64_REG_X4,
            "W5": unicorn.arm64_const.UC_ARM64_REG_X5,
            "W6": unicorn.arm64_const.UC_ARM64_REG_X6,
            "W7": unicorn.arm64_const.UC_ARM64_REG_X7,
            "W8": unicorn.arm64_const.UC_ARM64_REG_X8,
            "W9": unicorn.arm64_const.UC_ARM64_REG_X9,
            "W10": unicorn.arm64_const.UC_ARM64_REG_X10,
            "W11": unicorn.arm64_const.UC_ARM64_REG_X11,
            "W12": unicorn.arm64_const.UC_ARM64_REG_X12,
            "W13": unicorn.arm64_const.UC_ARM64_REG_X13,
            "W14": unicorn.arm64_const.UC_ARM64_REG_X14,
            "W15": unicorn.arm64_const.UC_ARM64_REG_X15,
            "W16": unicorn.arm64_const.UC_ARM64_REG_X16,
            "W17": unicorn.arm64_const.UC_ARM64_REG_X17,
            "W18": unicorn.arm64_const.UC_ARM64_REG_X18,
            "W19": unicorn.arm64_const.UC_ARM64_REG_X19,
            "W20": unicorn.arm64_const.UC_ARM64_REG_X20,
            "W21": unicorn.arm64_const.UC_ARM64_REG_X21,
            "W22": unicorn.arm64_const.UC_ARM64_REG_X22,
            "W23": unicorn.arm64_const.UC_ARM64_REG_X23,
            "W24": unicorn.arm64_const.UC_ARM64_REG_X24,
            "W25": unicorn.arm64_const.UC_ARM64_REG_X25,
            "W26": unicorn.arm64_const.UC_ARM64_REG_X26,
            "W27": unicorn.arm64_const.UC_ARM64_REG_X27,
            "W28": unicorn.arm64_const.UC_ARM64_REG_X28,
            "W29": unicorn.arm64_const.UC_ARM64_REG_X29,
            "W30": unicorn.arm64_const.UC_ARM64_REG_X30,
        ...
        self.regs.update({
            "arg1": unicorn.arm64_const.UC_ARM64_REG_X0,
        ...
        self.regs = {
            ...
            "PC": unicorn.arm_const.UC_ARM_REG_R15,
            "pc": unicorn.arm_const.UC_ARM_REG_R15,
            "LR": unicorn.arm_const.UC_ARM_REG_R14,
            "SP": unicorn.arm_const.UC_ARM_REG_R13,
            "sp": unicorn.arm_const.UC_ARM_REG_R13,
            "apsr": unicorn.arm_const.UC_ARM_REG_APSR,
            "APSR": unicorn.arm_const.UC_ARM_REG_APSR,
            "ret": unicorn.arm_const.UC_ARM_REG_R0,
        ...
        self.regs.update({
            "arg1": unicorn.arm_const.UC_ARM_REG_R0,
            "arg2": unicorn.arm_const.UC_ARM_REG_R1,
            "arg3": unicorn.arm_const.UC_ARM_REG_R2,
            "arg4": unicorn.arm_const.UC_ARM_REG_R3,
        })
      else:
        self.logger.error(
            "sample contains code for unsupported processor architecture"
    ...
    self.apiHooks["MultiByteToWideChar"] = (
        flare_emu_hooks._multiByteToWideCharHook
    )
    self.apiHooks["WideCharToMultiByte"] = (
        flare_emu_hooks._wideCharToMultiByteHook
    )
    ...
    self.logger.info(
        "initialized emulator for %s with %s architecture in %s mode"
  ...
  def reloadBinary(self):
    ...
    self.logger.debug(
        "base addr: %s end addr: %s memsize: %s"
        % (
            self.hexString(baseAddr),
            self.hexString(endAddr),
            self.hexString(memsize),
        )
    )
  ...
  def copyEmuMem(self, dstAddr, srcAddr, size, userData):
    ...
    try:
      ...
      self.logger.error(
          "exception in copyEmuMem @%s: %s"
          % (self.hexString(userData["currAddr"]), str(e))
      )

  def getCallTargetName(self, address):
    if self.analysisHelper.getOpndType(address, 0) == self.analysisHelper.o_reg:
      funcName = self.analysisHelper.getName(
          self.getRegVal(self.analysisHelper.getOperand(address, 0))
      )
    else:
      funcName = self.analysisHelper.getName(
          self.analysisHelper.getOpndValue(address, 0)
      )
  ...
  def _hookMemInvalid(self, uc, access, address, size, value, userData):
    self.logger.debug(
        "invalid memory operation for %s @%s"
    ...
    try:
      ...
      self.logger.debug(
          "error writing to %s, changing IP from %s to %s"
          % (
              self.hexString(address),
              self.hexString(userData["currAddr"]),
              self.hexString(userData["currAddr"] + userData["currAddrSize"]),
          )
      )
  ...
  def _hookInterrupt(self, uc, intno, userData):
    self.logger.debug(
        "interrupt #%d received @%s"
        % ((intno), self.hexString(userData["currAddr"]))
    )
    if self.arch == unicorn.UC_ARCH_X86:
      self.writeEmuMem(
          userData["currAddr"], X86NOP * int(userData["currAddrSize"])
      )
    elif self.arch == unicorn.UC_ARCH_ARM:
  ...
  def _handleApiHooks(self, address, argv, funcName, userData):
    ...
    try:
      ...
      self.logger.error(
          "error handling API hook: %s @%s" % (e, self.hexString(address))
      )
  ...
  def _emulateRangeCodeHook(self, uc, address, size, userData):
    try:
      ...
      if not userData.get("strict", True):
        if (
            self.analysisHelperFramework in ["Radare2", "Rizin"]
            or self.analysisHelper.getMnem(address) == ""
        ):
      ...
      if self.uc.mem_read(address, size) == b"\x00" * size:
        self.logger.debug(
            "pc ended up in null memory @%s" % self.hexString(address)
        )
        self.stopEmulation(userData)
        return
      ...
      # otherwise, stop emulation when returning from function emulation began in
          ...
          "funcStart" in userData
          and self.isRetInstruction(address)
          and self.analysisHelper.getFuncStart(address) == userData["funcStart"]
      ):
        self.stopEmulation(userData)
        return
      ...
      if self.analysisHelper.getMnem(address).upper() in self.callMnems or (
          self.analysisHelper.getMnem(address).upper() == "B"
          and self.analysisHelper.getNameAddr(
              self.analysisHelper.getOperand(address, 0)
          )
          == self.analysisHelper.getFuncStart(
              self.analysisHelper.getNameAddr(
                  self.analysisHelper.getOperand(address, 0)
              )
          )
      ):
        ...
        if userData["hookApis"] and self._handleApiHooks(
            address, self.getArgv(), funcName, userData
        ):
          return

        # skip calls if specified or there are no instructions to emulate at destination address
        if userData["skipCalls"] is True or (
            self.analysisHelper.getOpndType(address, 0)
            == self.analysisHelper.o_near
            and uc.mem_read(
                self.analysisHelper.getOpndValue(address, 0), self.size_pointer
            )
      ...
      # handle x86 instructions moving import pointers to a register
          ...
          self.analysisHelper.getMnem(address).lower() == "mov"
          and self.analysisHelper.getOpndType(address, 1)
          == self.analysisHelper.o_mem
          and self.analysisHelper.getOpndType(address, 0)
          == self.analysisHelper.o_reg
          and (
              len(self.analysisHelper.getOperand(address, 0)) == 3
              and (
                  self.analysisHelper.getOperand(address, 1)[:3] == "ds:"
                  or "sym.imp." in self.analysisHelper.getOperand(address, 1)
              )
              and uc.mem_read(
        ...
        uc.reg_write(
            self.regs[self.analysisHelper.getOperand(address, 0)],
            self.analysisHelper.getOpndValue(address, 1),
        )
        self.skipInstruction(userData)
      ...
      self.logger.error(
          "exception in emulateRangeCodehook @%s: %s"
          % (self.hexString(address), str(err))
      )
      self.stopEmulation(userData)
  ...
  def _emulateBytesCodeHook(self, uc, address, size, userData):
    try:
      ...
      self.logger.error(
          "exception in emulateBytesCodehook @%s: %s"
          % (self.hexString(address), str(err))
      )
      self.stopEmulation(userData)
    ...
    try:
      ...
      if address == bbStart and self.enteredBlock is True:
        if self.blockIdx < len(paths[self.pathIdx]) - 1:
          self.logger.debug(
              "loop re-entering block #%d (%s -> %s), forcing PC to %s"
              % (
                  self.blockIdx,
                  self.hexString(bbStart),
          ...
          self.logger.debug(
              "loop re-entering block #%d (%s -> %s), but no more blocks!"
              " bailing out of this function.."
        ...
        if self.blockIdx + 1 >= len(paths[self.pathIdx]):
          ...
          return
        self.logger.debug(
            "%s is outside of block #%d (%s -> %s), forcing PC to %s"
            % (
                self.hexString(address),
                self.blockIdx,
                self.hexString(bbStart),
      ...
      if self.analysisHelper.getMnem(address) == "":
        if self.analysisHelper.getMnem(address + size) == "":
          if self.analysisHelper.getMnem(address + size * 2) == "":
            self.logger.debug(
                "invalid instruction encountered @%s, bailing.."
                % self.hexString(address)
            )
            self.stopEmulation(userData)
          return
      ...
      if address == userData["targetVA"]:
        self.logger.debug(
            "target %s hit" % self.hexString(userData["targetVA"])
        )
        ...
        self.logger.debug(
            "target %s found on the way to %s, processing"
      ...
      self.logger.error(
          "exception in _guidedHook @%s: %s" % (self.hexString(address), e)
      )
      self.stopEmulation(userData)
  ...
  def _targetHit(self, address, userData):
    try:
      ...
      self.logger.error(
          "exception in targetCallback function @%s: %s"
          % (self.hexString(address), str(e))
      )
  ...
  def _isBadBranch(self, userData):
    if self.arch == unicorn.UC_ARCH_ARM64:
      if (
          self.analysisHelper.getMnem(userData["currAddr"]).upper()
          in ["BR", "BREQ"]
          and self.analysisHelper.getOpndType(userData["currAddr"], 0)
          == self.analysisHelper.o_reg
      ):
        if (
            self.analysisHelper.getMnem(
                self.getRegVal(
                    self.analysisHelper.getOperand(userData["currAddr"], 0)
                )
            )
        ) == "":
          return True
      ...
      if (
          self.analysisHelper.getMnem(userData["currAddr"]).lower() == "jmp"
          and self.analysisHelper.getOpndType(userData["currAddr"], 0)
          == self.analysisHelper.o_reg
      ):
        if (
          ...
          self.logger.debug(
              "bad branch detected @%s" % self.hexString(userData["currAddr"])
          )
          return True
    return False
  ...
  def _regionInSegment(self, addr, size):
    ...
    for seg in segs:
      if (
          (seg[0] <= addr and seg[1] > addr)  # starts in segment
          or (seg[0] <= addr + size and seg[1] > addr + size)  # ends in segment
          or (seg[0] > addr and seg[1] < addr + size)  # swallows segment
      ):
        return True
  ...
  def _findUnusedMemRegion(self, size):
    ...
    while candidate < maxAddr:
      if not self._regionOverlapsWithExistingRegion(
          candidate, size
      ) and not self._regionInSegment(candidate, size):
        break
  ...
  def _buildStack(self):
    self.stack = (
        self.allocEmuMem(self.stackSize) + self.stackSize // 2
    )  # fix for python3
  ...
  def _enableVFP(self):
    if self.arch == unicorn.UC_ARCH_ARM:
      ...
      MRS X2, CPACR_EL1
      ORR X2, X2, #0x300000 # <-- set bits 20,21 to disable trapping for FP
      related instructions
      MSR  CPACR_EL1, X2
      NOP # <-- handle Unicorn bug
      """
      ENABLE_VFP_CODE = (
          b"\x42\x10\x38\xd5\x42\x04\x6c\xb2\x42\x10\x18\xd5\x1f\x20\x03\xd5"
      )
      self.emulateBytes(ENABLE_VFP_CODE)
  ...
  def _prepEmuContext(self, registers=None, stack=None):
    ...
    if stack:
      for i in range(0, len(stack)):
        if (
            isinstance(stack[i], str)
            or isinstance(stack[i], bytes)
            or isinstance(stack[i], bytearray)
        ):
        ...
        self.writeEmuMem(
            self.getRegVal("sp") + i * self.size_pointer,
            struct.pack(self.pack_fmt, val),
        )
