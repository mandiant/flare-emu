...
def test_memory_access_hook(eh):
  ...
  if get_mov_types(eh, main_va) != userData["mov_types_hook"]:
    print(
        "FAILED: memory access hook test. Memory access identified in binary"
        " analysis and hooked instructions differ."
    )
  else:
...
def get_mov_types(eh, va):
  ...
  """
  mem_operand_types = [
      eh.analysisHelper.o_mem,
      eh.analysisHelper.o_phrase,
      eh.analysisHelper.o_displ,
  ]
...
if __name__ == "__main__":
  ...
  eh.iterate(
      eh.analysisHelper.getNameAddr(printfName), iterateHook, callHook=ch
  )
